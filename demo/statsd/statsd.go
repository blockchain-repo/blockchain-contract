...
func Example() {
	c, err := statsd.New(statsd.Address("192.168.80.22:8125")) // Connect to the UDP port 8125 by default.
	if err != nil {
...
func ExampleTagsFormat() {
	prex := statsd.Prefix("hcy")
	tag := statsd.Tags("region", "us", "app", "my_app")
	address := statsd.Address("192.168.80.22:8125")
	tagFormt := statsd.TagsFormat(statsd.InfluxDB)
	flushTime := statsd.FlushPeriod(time.Second)
	c, err = statsd.New(address,prex,tag,tagFormt,flushTime)
