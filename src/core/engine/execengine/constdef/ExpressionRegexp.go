...
var ExpressionRegexp = map[int]string{
	...
	Regexp_Condition: `[ ]*[!]*[ ]*[0-9a-zA-z_(,).\"\\']+([ ]*(>|<|==|!=|>=|<=|&&|\|\|)[ ]*[!]*[ ]*[0-9a-zA-z_(,).\"\\']+[ ]*)+`, // 条件表达式必须含有条件判断符号，不允许出现单目运算符（如!FuncAAAA()、!a），因为会与函数和变量表达式混淆
	Regexp_Name:      `[_a-zA-Z][a-zA-Z0-9_]*(.[_a-zA-Z][a-zA-Z0-9_]+)*`,
	Regexp_Func:      `Func[(\s\S)]+`,
	...
	Regexp_Name_Data_Int:           "data_int_",
	Regexp_Name_Data_Uint:          "data_uint_",
